...
class ChatToolBarClipper extends CustomClipper<Path> {
  @override
  Path getClip(Size size) {
    final path = Path();
    final double radius = 5;
    double width = size.width;
    double height = size.height;
    double marrginBottom = 8;
    final leftTopRect = Rect.fromLTRB(0, 0, radius * 2, radius * 2);
    // final leftBottomRect = Rect.fromLTRB(
    //     0, height - radius * 2, radius * 2, height - marrginBottom);
    final leftBottomRect = Rect.fromLTRB(0, height - radius * 2 - marrginBottom,
        radius * 2, height - marrginBottom);
    final rightTopRect =
        Rect.fromLTRB(width - 2 * radius, 0, width, radius * 2);
    final rightBottomRect = Rect.fromLTRB(width - 2 * radius,
        height - marrginBottom - radius * 2, width, height - marrginBottom);
    ...
    //右下角
    path.lineTo(width, height - radius - marrginBottom);
    path.arcTo(rightBottomRect, 0, 90 / 180 * pi, false);

    path.lineTo(size.width / 2 + marrginBottom, size.height - marrginBottom);
    path.lineTo(size.width / 2, size.height); //
    path.lineTo(size.width / 2 - marrginBottom, size.height - marrginBottom);
    //左下角
    path.lineTo(radius * 24, height - marrginBottom);
    path.arcTo(leftBottomRect, 90 / 180 * pi, 90 / 180 * pi, false);
